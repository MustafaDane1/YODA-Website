...
// Sadece aktif ekibin verisini (TEAM_DATA) alır
...
// ----------------------------------------------------------------
// TeamMemberCard BİLEŞENİ (Hizalama düzeltmeleri yapıldı)
// ----------------------------------------------------------------
const TeamMemberCard: React.FC<{ member: TeamMember }> = ({ member }) => {
  ...
  return (
    <div className="text-center px-2">
      ...
      {/* DEĞİŞİKLİK 1: Rol (Sarı Yazı)
        - 'h-11' (mobilde ~2 satır, 44px) 
        - 'sm:h-6' (webde 1 satır, 24px) 
        - 'overflow-hidden' (taşanı gizler)
      */}
      <p className="text-yoda-accent text-sm mb-2 leading-normal h-11 sm:h-6 overflow-hidden">
        {member.role}
      </p>
      
      {/* DEĞİŞİKLİK 2: Açıklama (Beyaz Yazı)
        - 'h-5' (mobilde ve webde ~1 satır, 20px) 
        - 'overflow-hidden' (taşanı gizler)
      */}
      <p className="text-gray-300 text-xs mb-4 leading-relaxed h-5 overflow-hidden">
        {member.description}
      </p>

      {/* 3. İSTEK: LinkedIn logosu (Zaten altındaydı) */}
...
// ----------------------------------------------------------------
// Ekibimiz BİLEŞENİ (Kodun geri kalanı aynı)
// ----------------------------------------------------------------
const Ekibimiz: React.FC = () => {
  ...
  // ID'ler 'ekibimiz' ve 'team-' olarak ayarlı
  const swiperStyles = `
    #ekibimiz .swiper {
      min-height: 500px;
    }
    ...
    <section id="ekibimiz" className="py-20 bg-[#000000]">
      ...
      <div className="container mx-auto px-4 sm:px-8">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-yoda-light uppercase tracking-wider">Ekibimiz</h2>
